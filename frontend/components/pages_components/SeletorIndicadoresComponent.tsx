...
export default function SeletorIndicadoresComponent({
    ...
    const [isCarregandoIndicadores, setIsCarregandoIndicadores] = useState(true);
    ...
    const referenciasDoIndicador = [2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009];
    ...
    function aplicarFiltros() {
        ...
        if (isIndicadorSelecionado && tipoGraficoSelecionado && isComparacaoValida) {
        ...
        <section className="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            <div className="mb-6 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">Combine um indicador educacional com o formato de gráfico mais útil para sua análise.</p>
                </div>
            </div>

            <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]">
                <div className="flex min-w-0 flex-col gap-2">
                    <label className="text-sm font-semibold text-slate-700" htmlFor="seletor-indicador">Indicador educacional</label>
                    <div className="relative">
                        <select
                            ...
                            disabled={isCarregandoIndicadores}
                        >
                        ...
                        {(["linha", "barra"] as TipoGrafico[]).map((tipo) => {
                            ...
                            return (
                                <button
                                    key={tipo}
                                    type="button"
                                    disabled={!isIndicadorSelecionado}
            ...
            {tentouAplicarFiltro && (!isIndicadorSelecionado || !tipoGraficoSelecionado || !isComparacaoValida) && (
                <div className="mt-5 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                    ...
                    <p>{!isIndicadorSelecionado ? "Selecione um indicador para continuar." : !tipoGraficoSelecionado ? "Escolha o tipo de visualização para gerar o gráfico." : "Selecione pelo menos 2 municípios para a comparação."}</p>
                </div>
            )}
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
                ...
                <button id="botao-aplicar-filtros" className="rounded-xl bg-[#061F56] px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-950/15 transition hover:-translate-y-0.5 hover:bg-[#0A3477] disabled:cursor-not-allowed disabled:opacity-50" onClick={aplicarFiltros} disabled={isCarregandoIndicadores}>
                    Gerar análise
                </button>
            </div>
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div className="flex-1">
                            <div className="flex items-center justify-between gap-3">
                                ...
                                <span className="rounded-lg bg-white px-3 py-1 text-sm font-bold text-sky-800 shadow-sm">{anoPrevisao}</span>
                            </div>
                            <input id="ano-previsao" type="range" min="2026" max="2040" step="1" value={anoPrevisao} onChange={(e) => setAnoPrevisao(Number(e.target.value))} className="mt-3 w-full accent-sky-700" />
                        </div>
                        <button className="rounded-xl border border-sky-200 bg-white px-5 py-3 text-sm font-semibold text-sky-800 transition hover:border-sky-400 hover:bg-sky-100">
                            Visualizar projeção
                        </button>
                    </div>
                </div>
